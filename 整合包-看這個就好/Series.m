%直接選取單筆實驗數據資料夾 或 data(全數資料的資料夾)
%資料彙總 讀取資料夾內同檔名資料ps"連接"並儲存
%建置_combined.mat並儲存
%選擇單筆實驗不影響(可用於查看box圖等) 全數據資料夾用以合併數據
...
data_list = {[ channel{index} '_base'],[ channel{index} '_fatigue'],[channel{index} '_recovered']};
...
for data_file = 1:length(data_list)
    ...
    % 在這裡保存或處理 total_ps
    save(fullfile(new_path, [data_list{data_file} '_combined.mat']), 'total_ps', 'f');
    clear total_ps;
end
end