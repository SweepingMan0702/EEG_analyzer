...
file_list = {[channel{index} '_base_combined.mat'],[channel{index} '_fatigue_combined.mat'],[channel{index} '_recovered_combined.mat']};
data_info = cell(3);
...
for band = 1:3  % 1: alpha, 2: beta, 3: theta
   ...
   %直方圖繪製
    ...
    pic_path = fullfile([fpath '\統計圖片'] , ['histogram_' channel{index} '_' bands{band} '.png']);
    if save_switch == 1
        saveas(gcf,pic_path);
        close all;
    end
    ...
    set(gcf, 'Position', get(0, 'Screensize'));
    pic_path = fullfile([fpath '\統計圖片'] , [ 'boxchart_' channel{index} '_' bands{band} '.png']);
    if save_switch == 1
        saveas(gcf,pic_path);
        close all;
    end
