...
load_data = load([fpath fname]);
load_data = load_data.data;
if size(load_data, 1) == 2
    eeg_data_Cz = load_data(1,:);
    eeg_data_Fz = load_data(2,:);
elseif size(load_data, 1) == 4
    eeg_data_Cz = load_data(2,:);
    eeg_data_Fz = load_data(3,:);
end
% 載入EEG數據
eeg_data_Cz = load_data(1,:);
eeg_data_Fz = load_data(2,:);
...
for i = 1:num_segments
    ...
    data = [eeg_data_Cz(:, start_sample:end_sample);
                    eeg_data_Fz(:, start_sample:end_sample)];
    
    % 創建文件名
    file_name = sprintf('segment_%d.mat',i);
    full_path = fullfile(fpath, file_name);
